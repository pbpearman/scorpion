...
load("/home/pearman/lud11_docs/wsl_research/projects/scorpion/data/ccsm_30sec/CCSM_CLIM_SP_DF")



predict.data1 <- ccsm.clim2
rm(ccsm.clim2)
...
for(k in species){
  ...
  fname = paste("CCSM_PREDICTION_",k,sep="")  
...
load("CCSM_PREDICTION_scorp")
dummy <- raster("/home/pearman/lud11_docs/wsl_research/projects/scorpion/data/ccsm_30sec/index_layer.tif")
values(dummy) <- NA
models <- c("gam","glm","gbm","me")
...
ccsm.distribution <- rasterize(predict.data,y=dummy,field="ensemble")

setwd("/home/pearman/lud11_docs/wsl_research/projects/scorpion/r_code/scorp/")
writeRaster(ccsm.distribution,"ccsm_distribution.tif",format="GTiff",overwrite=TRUE)
