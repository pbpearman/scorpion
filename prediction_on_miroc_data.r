...
load("/home/pearman/lud11_docs/wsl_research/projects/scorpion/data/miroc_30sec/MIROC_CLIM_SP_DF")



predict.data1 <- miroc.clim2
rm(miroc.clim2)
...
for(k in species){
  ...
  for(i in models){
    ...
    for(j in para.sets){
      ...
      rm(list=y)
      fname = paste("MIROC_PREDICTION_",k,sep="")  
...
load("MIROC_PREDICTION_scorp")
dummy <- raster("/home/pearman/lud11_docs/wsl_research/projects/scorpion/data/miroc_30sec/index_layer.tif")
...
miroc.distribution <- rasterize(predict.data,y=dummy,field="ensemble")

setwd("/home/pearman/lud11_docs/wsl_research/projects/scorpion/r_code/scorp/")
writeRaster(miroc.distribution,"miroc_distribution.tif",format="GTiff")
