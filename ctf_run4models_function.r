...
#                                                                             #
#                                                                             #
#   WRITTEN BY RAFAEL WUEEST, DEC. 2011, WSL BIRMENSDORF, SWITZERLAND         #
#   rafael.wueest@wsl.ch                                                      #
#                                                                             #
#   original code was provided by Peter B. Pearman, WSL Birmensdorf and       #
#   Manuela D'Amen, Rome, Italy                                               #
#   and transformed into a function by Rafael Wueest                          #
#                                                                             #
###############################################################################
...
#---------------
         if('gbm' %in% models){
            ...
            write.dbf(gbm.out.data,paste(direc,'/',spec,'_curr.pred_gbm_',para.names[i],'.dbf',sep=''))
...
#------------------
         if('gam' %in% models){
            ...
            for(v in 1:k){
               ...
               pred.occ.gam1 <- pred.occ.gam1[which(!is.na(pred.occ.gam1$gam.pred1)),] #inserted to account for an NA in predicted probabilities
            ...
            ## writing files into results directory
            write.dbf(gam.out.data,paste(direc,'/',spec,'_curr.pred_gam_',para.names[i],'.dbf',sep=''))
...
#------------------
         if('glm' %in% models){
            ...
            ## writing files into results directory
            write.dbf(glm.out.data,paste(direc,'/',spec,'_curr.pred_glm_',para.names[i],'.dbf',sep=''))
...
#---------------------
         if('me' %in% models){
            ...
            ## writing files into results directory
            write.dbf(me.out.data,paste(direc,'/',spec,'_curr.pred_me_',para.names[i],'.dbf',sep=''))
